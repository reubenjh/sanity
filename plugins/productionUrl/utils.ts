...
export async function getSecret(
  ...
  const secret = await client.fetch<string | null>(
    // Use a TTL of 10 hours when reading the secret, while using a 5 hour expiry if `createIfNotExists` is defined to avoid a race condition where
    ...
    query(createIfNotExists ? 600 * 30 : 600 * 60),
    {id}
  )
